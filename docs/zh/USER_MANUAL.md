...
| `cron`           | 定时任务：在固定时间执行预设提示（如每日索引检查）。 |
| `heartbeat`      | 心跳巡检：Agent 定期检查数据库健康并报告异常。 |
...
| `cron_chat_id` | string | 定时任务/心跳输出目标 chat_id（可选）。 |
| `heartbeat_chat_id` | string | 心跳输出目标；未设则用 cron_chat_id。 |
...
| `cron_chat_id`   | string | 定时任务/心跳输出目标（会话 ID 或用户 ID）。 |
| `heartbeat_chat_id` | string | 心跳输出目标；未设则用 cron_chat_id。 |
...
在固定时间执行自然语言提示，结果推送到通道或写入日志。

| 字段             | 类型   | 默认值 | 说明 |
|------------------|--------|--------|------|
| `enabled`        | bool   | `false`| 是否启用定时任务。 |
| `jobs`           | list   | `[]`   | 任务列表。 |
| `default_chat_id`| string | `""`   | 通道未单独配置时的默认 chat_id。 |
...
|-----------|--------|------|
| `id`      | string | 唯一任务 ID。 |
| `schedule`| string | `"at 09:00"`（每日）、`"every 30m"`、`"cron 0 9 * * 1"`（周一 9 点）。 |
| `prompt`  | string | 发给 Agent 的自然语言提示。 |
| `enabled` | bool   | 是否启用该任务。 |

### 心跳 (heartbeat)

定期健康检查：Agent 巡查数据库，仅在需要时报告。

| 字段               | 类型   | 默认值 | 说明 |
|--------------------|--------|--------|------|
| `enabled`          | bool   | `false`| 是否启用心跳。 |
| `interval_minutes` | int    | `30`   | 执行间隔（分钟）。 |
| `prompt`           | string | (默认) | 健康检查提示词。 |
| `default_chat_id`  | string | `""`   | 默认 chat_id。 |
  ...
  "jobs": [
    {
      ...
      "schedule": "at 09:00",
      "prompt": "检查是否有超过 10 万行的表缺少索引，列出它们。",
      "enabled": true
    }
  ]
},
...
- [定时任务设计](DESIGN_SCHEDULER.md)（[英文](../DESIGN_SCHEDULER.md)）— Cron 与 Heartbeat  
